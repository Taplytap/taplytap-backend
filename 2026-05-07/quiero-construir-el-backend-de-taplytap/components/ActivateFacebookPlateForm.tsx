...
import { getFacebookProfileUrlError } from "@/lib/facebook-url";

type ActivateFacebookPlateFormProps = {
  code: string;
...
export function ActivateFacebookPlateForm({
  code,
  currentUserEmail
}: ActivateFacebookPlateFormProps) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const frontendError = getFacebookProfileUrlError(destinationUrl);
    ...
    startTransition(async () => {
      const response = await fetch(`/api/activate/facebook/${code}`, {
        method: "POST",
        body: formData
      });
      ...
      if (!response.ok || result?.ok !== true) {
        ...
        setSubmitError(errorResult?.error ?? "No pudimos activar tu placa de Facebook.");
        return;
      }
    ...
    return (
      <>
        <section className="mt-8 rounded-2xl border border-line bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] animate-in fade-in slide-in-from-bottom-3 duration-700 sm:p-8">
          ...
          <h2 className="mt-6 text-2xl font-bold text-ink">Tu placa de Facebook ya está activa</h2>
          <p className="mt-3 text-sm leading-6 text-slateText">
            Tu placa ahora enviará a tus clientes directo al enlace de Facebook que configuraste.
          </p>
    ...
    <>
      <form onSubmit={onSubmit} className="mt-8 rounded-2xl border border-line bg-white p-5 shadow-[0_24px_70px_rgba(15,23,42,0.08)] sm:p-8">
        <div className="grid gap-5">
          ...
          <label className="grid gap-2">
            <span className="text-sm font-semibold text-ink">
              Enlace de tu perfil de Facebook <span className="text-error">*</span>
            </span>
            <input
            ...
            <span className="text-sm leading-6 text-slateText">
              Abre tu perfil en la aplicación de Facebook, toca “Compartir perfil”, copia el enlace y pégalo aquí.
            </span>
          </label>
        </div>

        <p className="mt-6 rounded-xl border border-line bg-white px-4 py-3 text-sm leading-6 text-slateText">
          ¿Ya tienes otra placa TaplyTap? Usa el mismo correo y contraseña para administrar tus placas de Google y Facebook desde una sola cuenta.
        </p>
        ...
        >
          ...
          {isPending ? "Activando..." : "Activar placa de Facebook"}
        </button>
      </form>
