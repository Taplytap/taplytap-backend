...
import type { FacebookPlate, QrStatus } from "@/lib/types";

type AdminFacebookPlateEditFormProps = {
  plate: FacebookPlate;
};
...
export function AdminFacebookPlateEditForm({ plate }: AdminFacebookPlateEditFormProps) {
  ...
  function save() {
    ...
    startTransition(async () => {
      const response = await fetch(`/api/admin/facebook-plates/${plate.code}`, {
        method: "PATCH",
        headers: {
    ...
    <div className="rounded-3xl border border-line bg-white p-5 shadow-sm sm:p-8">
      <div className="grid gap-5">
        ...
        <label className="grid gap-2">
          <span className="text-sm font-semibold text-ink">Enlace de Facebook</span>
          <input
        ...
        {status === "inactive" ? (
          <p className="rounded-xl border border-line bg-slate-50 px-4 py-3 text-sm leading-6 text-slateText">
            Si dejas la placa como pendiente, `/facebook/{plate.code}` volverá a mostrar activación. No se limpian propietario ni destino automáticamente.
          </p>
        ) : null}
        {status === "blocked" ? (
