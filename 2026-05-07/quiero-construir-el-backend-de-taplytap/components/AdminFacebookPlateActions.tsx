...
type AdminFacebookPlateActionsProps = {
  code: string;
...
export function AdminFacebookPlateActions({
  code,
  publicUrl,
  destinationUrl,
  status
}: AdminFacebookPlateActionsProps) {
  ...
  function updateStatus(nextStatus: "inactive" | "active" | "blocked") {
    startTransition(async () => {
      const response = await fetch(`/api/admin/facebook-plates/${code}`, {
        method: "PATCH",
        headers: {
