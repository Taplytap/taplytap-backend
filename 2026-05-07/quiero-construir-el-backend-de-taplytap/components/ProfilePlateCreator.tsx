...
export function ProfilePlateCreator() {
  ...
  async function generateLinks() {
    if (quantity >= 100) {
      const confirmed = window.confirm(`Vas a generar ${quantity} placas de Perfil permanentes. ¿Quieres continuar?`);
    ...
    startTransition(async () => {
      try {
        const response = await fetch("/api/admin/profile-plates/links", {
          method: "POST",
        ...
        if (!response.ok || !result || !("ok" in result)) {
          ...
          setError(errorResult?.error ?? "No pudimos generar los links de Perfil.");
          return;
        }

        endProgress();
        setLastDownload({ csv: result.csv, filePrefix: result.filePrefix });
        setMessage(`${result.count} placas de Perfil generadas correctamente`);
        downloadCsv(result.csv, result.filePrefix);
      } catch {
        setError("No pudimos generar las placas de Perfil. Intenta de nuevo.");
      } finally {
        if (progressTimer.current) clearInterval(progressTimer.current);
      }
    });
  ...
  async function exportLinksCsv() {
    ...
    startTransition(async () => {
      try {
        const response = await fetch("/api/admin/profile-plates/links/export", {
          method: "POST"
        });
        const result = (await response.json().catch(() => null)) as LinksResponse | { error?: string } | null;
        ...
        if (!response.ok || !result || !("ok" in result)) {
          const errorResult = result as { error?: string } | null;
          setError(errorResult?.error ?? "No pudimos exportar los links de Perfil.");
          return;
        }
        ...
        setMessage(`${result.count} links de Perfil exportados correctamente`);
        downloadCsv(result.csv, result.filePrefix);
      } catch {
        setError("No pudimos exportar los links de Perfil. Intenta de nuevo.");
      } finally {
        if (progressTimer.current) clearInterval(progressTimer.current);
      }
    });
    ...
    <div className="rounded-2xl border border-line bg-white p-4 shadow-sm">
      ...
      {isPending ? (
        <div className="mt-4">
          ...
          <p className="mt-2 text-xs text-slateText">Creando registros permanentes para placas Perfil...</p>
        </div>
      ) : null}
