...
import { ExternalLink, Share2, Link2, Pencil } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Card } from "@/components/ui/card";
import { cn } from "@/lib/utils";

export type FacebookPlateItem = {
  id: string;
  code: string;
  status: string;
...
type FacebookPlateSectionProps = {
  plates: FacebookPlateItem[];
};

export function FacebookPlateSection({ plates }: FacebookPlateSectionProps) {
  if (plates.length === 0) {
    return null;
  }
  ...
  return (
    <section className="taply-fade-up mt-10">
      ...
      <div className="grid gap-4">
        {plates.map((plate) => (
          <FacebookPlateCard key={plate.id} plate={plate} />
        ))}
      </div>
    </section>
  );
...
function FacebookPlateCard({ plate }: { plate: FacebookPlateItem }) {
    ...
    <Card className="overflow-hidden rounded-[2rem] border-white bg-white/95 p-5 shadow-[0_20px_70px_rgba(15,23,42,0.08)]">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h3 className="mt-4 text-xl font-bold tracking-tight text-ink">Placa Facebook</h3>
...
function FacebookUrlEditor({
  ...
  function saveDestinationUrl() {
    ...
    startTransition(async () => {
      try {
        const response = await fetch(`/api/dashboard/facebook-plates/${code}/destination`, {
          method: "PATCH",
          headers: {
      ...
      {isEditing ? (
        <div className="fixed inset-0 z-50 flex items-end bg-ink/35 px-4 py-5 sm:items-center sm:justify-center">
          <div className="w-full rounded-2xl border border-line bg-white p-5 shadow-[0_24px_80px_rgba(15,23,42,0.18)] sm:max-w-md">
            <div>
              <p className="text-sm font-semibold text-ink">Cambiar enlace de Facebook</p>
              <p className="mt-1 text-sm leading-6 text-slateText">
                Pega el enlace que Facebook te da al tocar “Compartir perfil”.
              </p>
            </div>
