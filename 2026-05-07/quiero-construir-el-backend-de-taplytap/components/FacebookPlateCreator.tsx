...
export function FacebookPlateCreator() {
  ...
  async function generateLinks() {
    if (quantity >= 100) {
      const confirmed = window.confirm(`Vas a generar ${quantity} placas de Facebook permanentes. ¿Quieres continuar?`);
    ...
    startTransition(async () => {
      try {
        const response = await fetch("/api/admin/facebook-plates/links", {
          method: "POST",
        ...
        if (!response.ok || !result || !("ok" in result)) {
          ...
          setError(errorResult?.error ?? "No pudimos generar los links de Facebook.");
          return;
        }

        endProgress();
        setLastDownload({ csv: result.csv, filePrefix: result.filePrefix });
        setMessage(`${result.count} placas de Facebook generadas correctamente`);
        downloadCsv(result.csv, result.filePrefix);
      } catch {
        setError("No pudimos generar las placas de Facebook. Intenta de nuevo.");
      } finally {
        if (progressTimer.current) clearInterval(progressTimer.current);
      }
    });
  ...
  async function exportLinksCsv() {
    ...
    startTransition(async () => {
      try {
        const response = await fetch("/api/admin/facebook-plates/links/export", {
          method: "POST"
        });
        const result = (await response.json().catch(() => null)) as LinksResponse | { error?: string } | null;
        ...
        if (!response.ok || !result || !("ok" in result)) {
          const errorResult = result as { error?: string } | null;
          setError(errorResult?.error ?? "No pudimos exportar los links de Facebook.");
          return;
        }
        ...
        setMessage(`${result.count} links de Facebook exportados correctamente`);
        downloadCsv(result.csv, result.filePrefix);
      } catch {
        setError("No pudimos exportar los links de Facebook. Intenta de nuevo.");
      } finally {
        if (progressTimer.current) clearInterval(progressTimer.current);
      }
    });
    ...
    <div className="rounded-2xl border border-line bg-white p-4 shadow-sm">
      ...
      {isPending ? (
        <div className="mt-4">
          ...
          <p className="mt-2 text-xs text-slateText">Creando registros permanentes para placas Facebook...</p>
        </div>
      ) : null}
