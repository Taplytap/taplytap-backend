...
import { AdminFacebookPlateActions } from "@/components/AdminFacebookPlateActions";
import { FacebookPlateCreator } from "@/components/FacebookPlateCreator";
import { StatusBadge } from "@/components/StatusBadge";
import { requireAdmin } from "@/lib/auth";
import { createSupabaseAdminClient } from "@/lib/supabase/admin";
import type { QrStatus } from "@/lib/types";

type AdminFacebookPageProps = {
  searchParams?: {
    page?: string;
    q?: string;
    status?: string;
...
export default async function AdminFacebookPage({ searchParams }: AdminFacebookPageProps) {
  await requireAdmin();
  ...
  const [{ data: plates, error: platesError, count }, totalCount, activeCount, inactiveCount, blockedCount] =
    await Promise.all([
      ...
      countFacebookPlates(),
      countFacebookPlates("active"),
      countFacebookPlates("inactive"),
      countFacebookPlates("blocked")
    ]);
    ...
    <main className="min-h-screen bg-[#F8FAFC] px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 rounded-3xl border border-line bg-white p-6 shadow-[0_18px_60px_rgba(15,23,42,0.06)] sm:p-8">
          ...
          <h1 className="mt-2 text-3xl font-bold tracking-tight text-ink">Placas Facebook</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slateText">
            Genera placas únicas para producción física de Facebook. Estos registros viven separados de
            las placas de reseñas y no cambian ningún QR público existente.
          </p>
        </div>

        <section className="mt-6">
          <FacebookPlateCreator />
        </section>
        ...
        <section className="mt-6 rounded-2xl border border-line bg-white p-5 text-sm leading-6 text-slateText shadow-sm">
          ...
          <p className="mt-2">
            ...
            <span className="font-mono text-xs text-ink">https://app.taplytap.io/facebook/[codigo]</span>.
          </p>
          <p className="mt-2">
            El CSV siempre sale desde registros guardados en Supabase. La configuración del destino de Facebook se
        ...
        <section className="mt-8">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-ink">Placas de Facebook</h2>
          ...
          <form className="mt-4 rounded-2xl border border-line bg-white p-4 shadow-sm" action="/admin/facebook">
                  ...
                  {(plates ?? []).length === 0 ? (
                    <tr>
                      <td className="px-4 py-8 text-center text-sm text-slateText" colSpan={7}>
                        No encontramos placas de Facebook.
                      </td>
                    </tr>
                  ) : (
                    (plates ?? []).map((plate) => (
...
async function countFacebookPlates(status?: QrStatus) {
  const supabase = createSupabaseAdminClient();
  let query = supabase
    .from("facebook_plates")
...
function Pagination({
  ...
  const buildHref = (nextPageNumber: number) => {
    ...
    return `/admin/facebook?${params.toString()}` as never;
  };
