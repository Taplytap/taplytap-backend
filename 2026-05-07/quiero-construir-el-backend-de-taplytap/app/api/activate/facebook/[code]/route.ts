...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  const destinationUrl = normalizeFacebookProfileUrl(String(formData.get("destination_url") ?? ""));
  const destinationError = getFacebookProfileUrlError(destinationUrl);
  ...
  if (!currentPlate) {
    return NextResponse.json({ error: "Facebook plate not found." }, { status: 404 });
  }
