...
import { getFacebookProfileUrlError, normalizeFacebookProfileUrl } from "@/lib/facebook-url";
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  ...
  if (!plate) {
    return NextResponse.json({ error: "No encontramos esta placa de Facebook." }, { status: 404 });
  }
  ...
  const nextDestinationUrl = normalizeFacebookProfileUrl(
    typeof body.destination_url === "string" ? body.destination_url : plate.destination_url ?? ""
  );

  if (nextDestinationUrl) {
    const destinationError = getFacebookProfileUrlError(nextDestinationUrl);
  ...
  if (nextStatus === "active" && !nextDestinationUrl) {
    return NextResponse.json(
      { error: "Una placa activa necesita un enlace válido de Facebook." },
      { status: 400 }
    );
  }
